...
Monte–Carlo evaluation relies on compiling the system model into intermediate data structures amenable to bit-parallel traversal. This chapter presents additional algorithmic and data-structure refinements to the \acrshort{pdag} that underpins \textsc{McScram}'s solver and shows how these advances enable specialized kernels for composite gates.
...
replaced by its \acrshort{dnf} of Eq.~(\ref{eq:k_of_n_or_of_ands}).  The expansion
introduces
\[
...
\[
  ...
  \qquad G_{0}=\text{\acrshort{pdag}}(\mathcal{M}),
\]
...
\begin{table}[t]
  ...
  \begin{tabular}{clp{5.7cm}cc}
    ...
    8 & Phase~V & Final coalescing pass; fixed-point \acrshort{nnf} is reached & \checkmark & \checkmark \\
    \bottomrule
  \end{tabular}
\end{table}
...
\[
  ...
  \frac{|G_0|}{|G_\ell|},
\]
where $|G|$ counts all gates in the \acrshort{pdag}.\footnote{%
...
\begin{itemize}
  ...
  \item Level~$C8$ maintains the compression despite additional passes
        to reach \acrshort{nnf}, indicating that late-phase expansions (e.g.,
...
\begin{figure}[hb]
    ...
    \caption{Relationship between average gate fan-in $f$ in the compiled \acrshort{pdag} and structural compression factor $\gamma$.  Points are colored by compilation flag; dashed lines show ordinary least–squares fits on log--log axes with slopes indicated in the legend.}
