...
The \acrfull{af} develops \acrshort{ccf} probabilities from set of failure ratios and the total component failure probability \cite{rasmuson_kelly_2008}. For a system with $m$ components, the alpha factors are defined as:
\[
...
\]
The \acrlong{af} is particularly useful because its parameters have direct statistical meaning and can be estimated from operational data.

\subsubsection{Multiple-Greek Letter (MGL) Model}
\acrfull{mgl} is used for systems with higher level of redundancy or k-out of-n logic. The multiple Greek letter model includes parameters for the conditional probabilities that the N+1-th subsystem fails given that N identical subsystems have already failed \cite{jones_2012}. The key parameters are:
\begin{itemize}
...
The \acrfull{bfr} is based on the concept of two distinct types of shock events affecting components in a \acrshort{cccg}:
...
The \acrshort{bfr} model has the advantage of using physically interpretable parameters ($\nu$ and $p$) while requiring fewer parameters than the Basic Parameter Model.
...
Relationships exist between the parameters of different \acrshort{ccf} models, allowing conversion of parameters from one model to another. For example, the Alpha factors can be derived from \acrshort{mgl} parameters as:
\[
\begin{aligned}
